...
This program calculates a student's final course grade based on a strict, non-standard syllabus structure. It processes input for a final exam, a midterm, and five projects, and computes the final score using two specific grading policies:
...
This program is a collection of four standalone computational tools that perform mathematical and binary operations strictly from scratch, bypassing standard math libraries.
...
**Project 4: C-String Toolkit and "Autocorrect" Engine**
This program functions as a custom text-manipulation toolkit and a rudimentary spell-checker, operating entirely on raw character arrays (char*).
...
Levenshtein Distance Engine: This is the core "autocorrect" feature. It uses recursion to calculate the exact number of single-character edits (insertions, deletions, or substitutions) required to morph one word into another. If the software searches a dictionary array and cannot find an exact match for a word, it uses this mathematical distance to return the closest possible suggestion.

**Project 5: Linked-List Mathematical Set**
This program implements a complete, object-oriented Mathematical Set—a data structure that holds integers but strictly forbids any duplicate values.
...
Relational Comparisons: It uses comparison operators to evaluate the relationship between two different sets, mathematically verifying if they are identical (==), or if one set completely contains all the elements of another (Subset/Superset via <= and >=).
